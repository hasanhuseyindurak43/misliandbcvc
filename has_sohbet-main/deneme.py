...
class SolUst1(ctk.CTkFrame):
    def __init__(self, master, sag_orta, **kwargs):
        super().__init__(master, **kwargs)
        self.configure(height=245, width=245)
        self.grid_propagate(False)
        self.sag_orta = sag_orta
    ...
    def show_message_window(self, message, duration=3000):
        # Yeni CTkToplevel penceresi oluştur
        message_window = ctk.CTkToplevel(self)  # self, ana pencere referansı olmalı
        message_window.geometry('400x200')
        message_window.title('Bilgi')

        # Mesajı içeren bir etiket oluştur
        label = ctk.CTkLabel(message_window, text=message, font=('Arial', 18))
        label.pack(expand=True, fill='both')

        # Pencereyi kapatacak bir fonksiyon tanımla
        def close_window():
            message_window.destroy()

        # Pencereyi belirli bir süre sonra kapat
        message_window.after(duration, close_window)
    ...
    def qr_kod_oku(self):
        ...
        self.after(2, self.qr_kod_oku)

    def should_process_qr_code(self, qr_kod_data):
        current_time = datetime.now().time()
        current_date = datetime.now().date()

        first_time_slot = (dt_time(22, 45), dt_time(22, 50))
        second_time_slot = (dt_time(22, 55), dt_time(23, 0))
        ...
        if is_in_first_slot:
            ...
            self.show_employee_check_message(qr_kod_data, 'giriş yaptı')
        elif is_in_second_slot:
            cursor.execute(
                "INSERT OR REPLACE INTO qr_read_log (qr_code_data, read_date, read_time, read_slot) VALUES (?, ?, ?, ?)",
                (qr_kod_data, current_date.strftime("%Y-%m-%d"), current_time.strftime("%H:%M:%S"), 'second'))
            self.show_employee_check_message(qr_kod_data, 'çıkış yaptı')
    ...
    def check_employee(self, qr_kod_data):
        ...
        if result:
            self.sag_orta.update_result(f'{first_name} {last_name} adlı çalışan giriş yaptı.')
        else:
            self.sag_orta.update_result('Bu bilgilerle eşleşen bir çalışan bulunamadı.')


        conn.close()

    def show_employee_check_message(self, qr_kod_data, action):
        try:
            id_str, name = qr_kod_data.strip().split(',', 1)
            first_name, last_name = name.split(' ', 1)
            message = f"{first_name} {last_name} {action}"
            self.show_message_window(message)  # Updated to call the instance method
        except ValueError:
            self.result_label.configure(text='Hatalı QR kod verisi.')
...
class PanelTwo(ctk.CTk):
    def __init__(self):
        ...
        self.solust = SolUst1(master=self, sag_orta=self.sagorta)
