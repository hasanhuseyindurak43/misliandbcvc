...
def export_to_py():
    ...
    if file_path:
        ...
        with open(file_path, "w") as py_file:
            for line in data["list_1"]:
                py_file.write(line + "\n")
            for cmd in data["to_liste"]:
                py_file.write(cmd + "\n")
            for line in data["three_liste"]:
                py_file.write(line + "\n")
...
def update_json():
    ...
    for button in buttons:
        ...
        button_cmd = f"button = ctk.CTkButton(root, text='{button_text}', width={button_width}, height={button_height})\nbutton.place(x={new_x}, y={new_y})"
        to_liste.append(button_cmd)
        
        # Düzenleme için kullanılacak komutlar
        button_cmd_edit = f"create_button({new_x}, {new_y}, {button_width}, {button_height}, '{button_text}')"
